    ...
    if($_SERVER['REQUEST_METHOD']=='GET'){
        ...
        switch($_GET['name']){
            case 'org_name':
                ...
                $query = "select org_name from organizations where org_name in ('".$value."', '".strtolower($value)."', '".ucfirst($value)."', '".ucwords($value)."', '".strtoupper($value)."');";
                $result = $con->query($query);
                if($result->num_rows>0){
                    $output["error"]="Name already exist.";
                }else{
                    $output["status"]=true;
                    $output["result"]="Name available";
                }
                break;
            case 'phone_num':
                $output["status"]=true;
