<?php
    ...
    $query="select * from event_".$event_id."_pro_don inner join civilian_detail on civilian_detail.NIC_num = event_".$event_id."_pro_don.by_person where to_person='".$id."' AND (pro_don='promise' OR pro_don='pending');
    select * from event_".$event_id."_pro_don inner join organizations on organizations.org_id = event_".$event_id."_pro_don.by_org where to_person='".$id."' AND (pro_don='promise' OR pro_don='pending');
    select * from event_".$event_id."_pro_don inner join civilian_detail on civilian_detail.NIC_num = event_".$event_id."_pro_don.by_person where to_person='".$id."' AND pro_don='donated' ;
    select * from event_".$event_id."_pro_don inner join organizations on organizations.org_id = event_".$event_id."_pro_don.by_org where to_person='".$id."'AND pro_don='donated' ;
    ...
    if(mysqli_multi_query($con,$query)){
        ...
        $result = mysqli_store_result($con);
        $promise_org_detail = mysqli_fetch_all($result,MYSQLI_ASSOC);
        ...
        mysqli_next_result($con);
        $result = mysqli_store_result($con);
        $donated_org_detail = mysqli_fetch_all($result,MYSQLI_ASSOC);
        mysqli_free_result($result);
...
<div id="title">
    ...
    <table id='promise_table'>
        ...
        <?php
                    ...
                    foreach($promise_org_detail as $row_req){
                        $item_amount="";
                        ...
                        if ($status=="promise"){
                            $to=$id;
                            $by=$row_req['id'];
                            echo "  <tr onclick='edit_promise(\"/event/help/?event_id=".$event_id."&by=".$by."&to=".$to."\")'>
                                        <td>".$name."</td><td>".$item_amount."</td>
                                        <td>".$note."</td>
                                        <td class='not_click'>
                                        <input type='checkbox' data-toggle='toggle' data-on='Helped' data-off='Not helped' data-width='100' data-height='15' data-offstyle='warning' data-onstyle='success' onchange=''>
...
<div id="title">
    ...
    <table id='donated_table'>
        ...
        <?php
                    ...
                    foreach($donated_org_detail as $row_req){
                        $item_amount="";
                        $name=$row_req['org_name'];
                        $note=$row_req['note'];
                        foreach($content_detail as $row_req1){
